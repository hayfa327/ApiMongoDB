...
const createPerformance = async (req, res) => {
  try {
    const { title, description, type, image, artist, coArtist, eventDate, location, isLive } = req.body;
    ...
    const performance = await Performance.create({
      title, description, type, image, artist, coArtist, eventDate, location, isLive,
    });
...
const getAllPerformances = async (req, res) => {
  try {
    // Optional ?type=performance or ?type=concert to filter, used by the
    // separate Performances and Concerts pages on the frontend
...
const updatePerformance = async (req, res) => {
  try {
    ...
    // Only admin can reassign the artist on an existing performance
